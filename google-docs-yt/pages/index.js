...
export default function Home() {
  ...
  const createDocument = () => { 
    ...
    db.collection("userDocs").doc(session.user.email).collection("docs").add({
      fileName:input, timestamp: firebase.firestore.FieldValue.serverTimestamp()
    });
    setInput("");
    setShowModal(false);
  };
  ...
  const modal = (
    <Modal size="sm" active={showModal} toggler={()=>setshowModel(false)}>
        <ModalBody>
        ...
        <ModalFooter>
          <Button color="blue" buttonType="link" onClick={(e)=>setShowModal(false)}>
            Cancel
          </Button>
